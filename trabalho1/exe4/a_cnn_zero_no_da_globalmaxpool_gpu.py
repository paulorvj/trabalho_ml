...
MODEL_PATH = "cnn_cifar10_global_max_pooling_best.pth"
...
class CIFAR10Dataset(Dataset):
    ...
    def __getitem__(self, indice):
        ...
        # Aumento de dados somente no conjunto de treinamento
        if self.augmentation:
            # Inversão horizontal
            if random.random() < 0.5:
                imagem = np.fliplr(imagem).copy()

            # Pequenos deslocamentos horizontais e verticais
            if random.random() < 0.5:
...
def main():
    ...
    plt.title("Matriz de confusão — CNN com Global Max Pooling")
    plt.tight_layout()
    plt.savefig("cnn_cifar10_global_max_pooling_confusion_matrix.png", dpi=300)
    #plt.show()
    ...
    plt.xlabel("Época")
    plt.ylabel("Precisão")
    plt.title("Precisão — CNN com Global Max Pooling")

    plt.legend()
    plt.grid()

    plt.tight_layout()
    plt.savefig("cnn_cifar10_global_max_pooling_accuracy.png", dpi=300)
    #plt.show()
    ...
    plt.xlabel("Época")
    plt.ylabel("Loss")
    plt.title("Loss — CNN com Global Max Pooling")

    plt.legend()
    plt.grid()

    plt.tight_layout()
    plt.savefig("cnn_cifar10_global_max_pooling_loss.png", dpi=300)
    #plt.show()
